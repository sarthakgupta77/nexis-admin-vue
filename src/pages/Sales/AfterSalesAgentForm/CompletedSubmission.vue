...
const filterByEmail = () => {
  if (text.value.length >= 3) {
    api.post('pending-completed-submissions', { user_id: u_id, approval: 1, email: text.value }).then((res) => {
    ...
    api.post('pending-completed-submissions', { user_id: u_id, approval: 1 }).then((res) => {
      data.submissions = res.data.data.data
      count = 1
    })
...
const paginate = async (current) => {
  ...
  await api.post("https://genesis.datatrained.com/api/admin/pending-completed-submissions?page=" + current, { user_id: u_id, approval: 1 }).then((response) => {
...
onBeforeMount(() => {
  ...
  api.get('course').then((res) => {
    ...
    api.post('pending-completed-submissions', { user_id: u_id, approval: 1 }).then((response) => {
      data.submissions = response.data.data.data;
