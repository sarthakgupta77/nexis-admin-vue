      ...
      <div class="row">
        <div class="col-md-12">
          <div class="head-btn-box shadowBoder row">
            <h5 class="q-ma-none q-pt-xs" id="head-1">My Assigned Projects</h5>
            <div class="row col-6" style="justify-content: flex-end">
              <q-select label="Search By" @update:model-value="count = 1" :options="searchOptions" v-model="searchBy"
                label-color="primary" outlined dense class="q-px-sm" style="width: 18%" />
            ...
            <table id="dtBasicExample" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
              ...
              <tbody>
                <tr v-for="project in data.projects">
                  ...
                  <td>
                    <q-icon name="person" size="xs" color="primary" class="q-mr-xs" /> {{ project.name }} <br>
                  ...
                  <td>
                    <div v-if="project.submission == 0" class="badge-btn">
                      <b>Status :</b>
                      <q-chip color="red-5" class="text-center" size="12px" text-color="white">
                        Not Submitted
                      </q-chip>
                      <br />
                    </div>
                    <div v-else-if="project.submission == 1">
                      <div class="badge-btn">
                        <b>Status :</b> <q-chip color="indigo" class="text-center" size="12px" text-color="white">
                          Submitted
                        </q-chip>
                      </div>
                      <b>Date : </b> {{ (project.submission_date).split(' ')[0] }}
                      <br />
                    </div>
                  </td>
                  <td v-if="state.myassigned_view">
  ...
  if (searchBy.value == 'Email') {
    if (emailSearch.value.length >= 3) {
      api.get("my-assigned-records", {
        ...
        data.projects = response.data.records
