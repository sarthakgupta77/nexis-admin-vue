      ...
      <div class="row">
        <div class="col-md-12">
          <div class="head-btn-box row shadowBoder">
            <div class="row col-3">
              <h5 class="q-ma-none q-pt-xs" id="head-1">All Evaluated Projects</h5>
            </div>
            <div class="row col-4" style="justify-content: flex-end">
              <q-select outlined dense class="q-mr-sm col-5" label="Search By" label-color="primary" v-model="searchBy"
            ...
            <table id="dtBasicExample" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
              <thead class=" bg-grey-4">
                <tr class="text-left">
                  ...
                  <th scope="col" style="width: 15%">Project Submission</th>
                  <th v-if="state.allevaluated_viewscore" scope="col">Scores</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="project in data.projects">
                  <td class="text-left">{{ count++ }}</td>
                  <td> <q-icon name="person" size="xs" color="primary" class="q-mr-xs" /> {{ project.stu_name }} <br>
                  ...
                  <td>{{ project.batch }}</td>
                  <td> <q-icon name="person" size="xs" color="green" class="q-mr-xs" /> {{ project.evaluator_name }} <br>
                  ...
                  <td v-if="state.allevaluated_viewscore">
                    <q-btn flat round icon="open_in_browser" color="primary"
                      @click="viewProject(student_id = project.student_id, submission = project.submission, project_id = project.project_id)">
                      <q-tooltip anchor="top middle" self="bottom middle" :offset="[10, 10]" transition-show="scale"
                        transition-hide="scale">View Project Scores</q-tooltip>
                    </q-btn>
                  </td>
                </tr>
              </tbody>
            </table>
            <div class="q-py-md col-md-12 ">
...
const state = reactive({
  ...
  allevaluated_viewscore: ref(false),
})
...
const filterByEmail = () => {
  ...
  if (text.value.length >= 3) {
    ...
    api.get("all-evaluated-records", { params: { email: text.value, action: 1 } }).then((response) => {
...
const filterByBatch = () => {
  ...
  if (text.value.length >= 3) {
    count = 1;
    api.get("all-evaluated-records", { params: { batch: text.value, action: 1 } }).then((response) => {
      data.projects = response.data.projects;
    }).finally(() => {
      count = 1
    });
