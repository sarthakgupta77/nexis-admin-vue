      ...
      <div class="row">
        <div class="col-md-12">
          <div class="head-btn-box shadowBoder">
            <h5 class="q-ma-none q-pt-xs" id="head-1">All Assignments</h5>
            <q-btn v-if="state.create" class="create-btn" to="/addAssignments" label="Create New"></q-btn>
          </div>
          ...
          <div class="table-container shadowBoder">
              ...
              <div style="background: #fff; width: 550px;">
                <q-splitter :limits="[30, 100]" v-model="splitterModel" style="height: 100%;">
                  ...
                  <template v-slot:after>
                    ...
                    <h4 v-if="tab1 == ''" class="text-center q-ma-md text-primary w600"> Select A Action To Manage</h4>
                    <q-tab-panels v-model="tab1" animated swipeable vertical transition-prev="jump-up"
                      transition-next="jump-up">
                      <q-tab-panel name="assign" class="q-pa-none">
                        <q-form @submit="actions(assign = 0)" @reset="resetForm">
                          ...
                          <q-card-section>
                            <div class="row">
                              <div class="col-md-6">
                                ...
                                <q-select v-else disable outlined dense class="col-md-12 q-mr-sm" label="Select Batch" />
                              </div>
                              <div class="col-md-6">
                          ...
                          <q-card-actions align="right" style="padding: 20px;">
                            <q-btn label="Close" color="grey-8" type="reset" v-close-popup />
                      ...
                      <q-tab-panel name="unassign" class="q-pa-none">
                        <q-form @submit="actions(assign = 1)" @reset="resetForm">
                          <q-card-section class="row">
                            <q-toolbar>
                          ...
                          <q-separator />
                          <q-card-actions align="right" style="padding: 20px;">
                            <q-btn label="Close" color="grey-8" v-close-popup type="reset" />
                      ...
                      <q-tab-panel name="extend" class="q-pa-none">
                        <q-form @submit="actions(assign = 2)" @reset="resetForm">
                          <q-card-section class="row ">
                            <q-toolbar>
                          ...
                          <q-card-section>
                            <div class="row">
                              ...
                              <div class="col-md-6">
                                ...
                                <q-input dense outlined type="date" color="secondary" placeholder="dd-mm-yyyy"
                                  class="col-md-12 q-mr-sm" v-model="date" lazy-rules
                                  :rules="[val => val && val.length > 0 || 'Please Select New Due Date']" />
                              </div>
                            </div>
                            <q-separator />
                            <div class="col-md-12 q-pt-md">
            ...
            <div v-if="data.assignments.length > 0" class="q-pa-sm w600"> Total Records : {{ data.assignments.length }}
            </div>
            <table id="dtBasicExample" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
              <thead class="bg-grey-4">
                <tr class="text-left ">
                  ...
                  <th class="text-center" v-if="state.edit || state.view" scope="col">Action</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="assignments in data.assignments" :key="assignments.id">
                  <td class="text-left">{{ count++ }}</td>
                  <td>
                    <q-item dense class="text-primary" clickable :to="`/view-assignment/` + assignments.id"> {{
                      assignments.title }} </q-item>
                  </td>
                  <td class="text-left">
                    {{ assignments.course }}
                  </td>
                  <td class="text-center text-black">
                    <div v-if="assignments.type == 1">
                      <q-chip color="green-5" text-color="white">
                        Evaluation
                      </q-chip>
                      <br />
                    </div>
                    <div v-else-if="assignments.type == 0">
                      <q-chip color="blue-5" text-color="white" style="padding: 0px 20px;">
                        Practice
                      </q-chip>
                      <br />
                    </div>
                    <div v-else-if="assignments.type == 2">
                      <q-chip color="grey-5" text-color="white" style="padding: 0px 32px;"> Extra </q-chip>
                      <br />
                    </div>
                  </td>
                  <td class="text-center">
                    <div>
                      <q-btn flat rounded color="primary" icon-right="visibility" @click="assignedTo(id = assignments.id)"
                        label="Count" />
                    </div>
                  </td>
                  <td class="text-center">
                    <q-btn flat color="primary" rounded icon="assignment_ind"
                      @click="actionModal(project_id = assignments.id, type = assignments.type)" direction="left"
                      padding="sm" />
                  </td>
                  <td v-if="state.edit" class="text-center">
                    <q-fab flat color="grey-9" icon="settings" direction="left" padding="sm">
                      <q-fab-action @click="EditAssingment(assignments.id)" color="primary" text-color="white"
                        icon="edit">
...
onBeforeMount(() => {
  ...
  axios
    .get("https://genesis.datatrained.com/api/admin/assignments")
    .then((response) => {
...
.evl-1 {
  background-color: #5ba3ec;
  background-color: #398bdd;
  padding: 5px;
}

.prac-1 {
  background-color: #22d219;
  padding: 5px;
}

.view-btn:hover {
  color: #549ec9 !important;
}
